...
/// <summary>
/// ЛР14: Создаёт Mock-фабрику, которая при каждом вызове CreateDbContext()
/// возвращает новый InMemory-контекст с одним и тем же именем БД.
/// Это имитирует поведение IDbContextFactory в реальном приложении:
/// каждый контекст — независимый, но работает с одной и той же БД.
/// </summary>
static MockDbContextFactory CreateMockFactory()
{
    var dbName = Guid.NewGuid().ToString();
    return new MockDbContextFactory(dbName);
}

/// <summary>
/// Фабричный метод для создания ContactsListViewModel со всеми mock-зависимостями.
/// ЛР14: Вместо PhoneBookDbContext передаётся MockDbContextFactory.
/// </summary>
...
/// <summary>
/// ЛР14: Mock-фабрика для IDbContextFactory.
/// Каждый вызов CreateDbContext() создаёт новый экземпляр контекста,
/// работающий с одной и той же InMemory-базой данных.
/// Это позволяет тестировать паттерн «контекст на операцию» без реальной СУБД.
/// </summary>
public class MockDbContextFactory : IDbContextFactory<PhoneBookDbContext>
{
    private readonly string _databaseName;

    public MockDbContextFactory(string databaseName)
    {
